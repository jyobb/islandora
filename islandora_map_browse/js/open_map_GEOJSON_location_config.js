...
function initialize() {
    ...
    var position = new OpenLayers.LonLat(centreLocation.x, centreLocation.y).transform( fromProjection, toProjection);
    var zoom           = 8; 
    ...
    //    j = JSON.parse(displayfields);
    //console.log(JSON.stringify(j));
    //if("mods_location_url_ms" in j){
    //	console.log("test");
    //}
    ...
    var GEOvector = new OpenLayers.Layer.Vector("Points",{
		...
		'featureselected':function(evt){
			...
			//for(var u in evt.feature.cluster[key1].attributes){
			//    if(u in j){
			//	console.log("hay"+u);
			//    }
			//}
			...
			for(var u in evt.feature.cluster[key].attributes){
			    if(u in j){
				if(evt.feature.cluster[key].attributes[u] != "")
				    //console.log("wow"+evt.feature.cluster[key].attributes[u]);
				undercontent = undercontent + "<h4>"+ evt.feature.cluster[key].attributes[u] +"</h4>";
			    }
			}

			popupContent = popupContent + "<div style='border-bottom:1px solid black'>" +
        "<h2><a href='"+baseUrl+"/"+obj+"'>"+ title +"</a></h2>"+
        "<img width='254' height='355' src='"+baseUrl+"/"+thumb+"'>" + undercontent
